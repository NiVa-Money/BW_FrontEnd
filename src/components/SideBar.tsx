...
const MenuItem: React.FC<MenuItemProps> = ({ item, onClick, collapsed, setIsCollapsed }) => {
  ...
  const toggleSubMenu = () => {
    if (collapsed) {
      setIsCollapsed(false);  // Expand the sidebar if it is collapsed
    }
      ...
      {item.hasDropdown ? (
        <>
          ...
          >
            ...
            {!collapsed && (
              <i
                className={`fas fa-chevron-right ml-auto transition-transform ${
                ...
                <React.Fragment key={idx}>
                  <button
                    ...
                    className={`flex items-center space-x-2 py-2 px-3 text-gray-300 hover:bg-white hover:bg-opacity-10 rounded-full cursor-pointer ${
                      pathname && pathname.includes(subItem.path || '')
                        ? 'bg-white bg-opacity-10'
                        : ''
                    }`}
                  >
                    ...
                    {subItem.hasDropdown && (
                      <i
                        className={`fas fa-chevron-right ml-auto transition-transform ${
                          subMenuChildOpen[idx] ? 'transform rotate-90' : ''
                        }`}
                      ></i>
                    )}
                  </button>
                  {subMenuChildOpen[idx] && (
                    <div className="pl-4 mt-2 space-y-1">
                      {subItem.subChildItems?.map((childItem:any, childIdx:any) => (
                        <button
                          key={childIdx}
                          onClick={() =>
                            botSession(childItem.botId, childItem.userId, childItem.title)
                          }
                          className={`flex items-center space-x-2 py-2 px-3 text-gray-300 hover:bg-white hover:bg-opacity-10 rounded-full cursor-pointer ${
                            pathname && pathname.includes(childItem.path || '')
                              ? 'bg-white bg-opacity-10'
                              : ''
                          }`}
                        >
