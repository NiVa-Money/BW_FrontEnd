'use client';
import { getWhatsAppWebhookAction, saveWhatsAppAction } from '@/redux/actions/socialIntegrations/whatsAppIntegration';
...
const WhatsAppIntegration = () => {
  ...
  useEffect(() => {
    if (botDataRedux?.length) {
      const data:any = botDataRedux.map(
        ...
        const filteredBots = whatsappIntegratedBots?.length? data.filter((bot:any) => !whatsappIntegratedBots.some((item:any) => item.botId === bot.id)):data;
  ...
  useEffect(() => {
    console.log('g')
    dispatch(getWhatsAppWebhookAction('66f45f2342e89d89dfd60ada'));
  }, []);
  const {
    provider,
    bot,
