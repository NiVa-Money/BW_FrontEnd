...
const Modal: React.FC<ModalProps> = ({ closeModal, handleSignUp }) => {
  ...
  const validateForm = () => {
    ...
    if (!namePattern.test(firstName)) {
      newErrors.firstName =
        'First name should be more than 1 letter and contain no special characters or numbers.';
    }
    if (!namePattern.test(lastName)) {
      newErrors.lastName =
        'Last name should be more than 1 letter and contain no special characters or numbers.';
    }
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black">
          <div className="p-6 rounded-lg max-w-lg mx-auto relative">
            ...
            <form onSubmit={handleSubmit}>
              ...
              {errors.mobileNo && (
                <div className="text-red-500">{errors.mobileNo}</div>
              )}
              <button
                type="submit"
