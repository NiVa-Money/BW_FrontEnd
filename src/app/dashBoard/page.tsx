    ...
    const fetchData = async () => {
      try {
        ...
        const metricsResponse = await axios.get(
          `http://13.235.189.116:8000/user/metrics/${userId}`,
          {
            headers: {
        ...
        const userProfileResponse = await axios.get(
          `http://13.235.189.116:8000/user/getUserProfile?emailId=${encodeURIComponent(
            emailId
          )}`,
          {
            headers: {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className="flex flex-wrap justify-between gap-6">
        <BarListCard firstName={metrics.firstName} emailId={metrics.emailId} />
